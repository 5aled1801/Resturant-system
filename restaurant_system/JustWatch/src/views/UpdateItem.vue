      ...
      <div class="form-box">
        <form class="form" @click.prevent>
          ...
          <div style="display: flex; justify-content: space-around">
            <button style="background-color: rgb(2, 97, 2) !important" @click="submitForm()">
              Update Now
            </button>
...
export default {
  ...
  methods: {
    async submitForm() {
      this.v$.$validate()
      if (!this.v$.$error) {
        let addItem = await axios.put(`http://localhost:3000/item/${this.itemId}`, {
          name: this.name,
          userId: this.userId,
  ...
  async mounted() {
    ...
    let requestItemData = await axios.get(`http://localhost:3000/item/${this.itemId}`)
    this.listofitem = requestItemData.data
    console.log(requestItemData)
    this.name = this.listofitem.name
    this.price = this.listofitem.price
    this.des = this.listofitem.description
    this.qnt = this.listofitem.qtt
    this.catId = this.listofitem.categoryId
  }
}
</script>
<style>
